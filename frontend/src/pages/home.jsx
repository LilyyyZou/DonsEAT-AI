...
import { Search, MapPin, DollarSign, Star, ChevronDown, Zap, Camera, UserCircle } from 'lucide-react';
...
const MockCampusDishes = [
  {
    ...
    price: "$12.50",
    aiSummary: "AI Insight: This signature dish features slow-cooked chicken broth that students rate as the best on campus. High in protein, low fat, and uses 100% gluten-free buckwheat flour.",
  ...
  },
  {
    ...
    price: "$10.00",
    aiSummary: "AI Insight: A cult favorite at Lone Mountain. The 24-hour aged cheddar combined with house-made truffle aioli provides a premium experience for a campus meal.",
    ingredients: ["Grass-fed Angus Beef", "Truffle Aioli", "24-hour Aged Cheddar", "Brioche Bun"],
  ...
  },
  {
    ...
    price: "$14.50",
    aiSummary: "AI Insight: One of the most nutritionally balanced meals available. The Atlantic salmon is omega-3 rich and typically served with brown rice.",
    ingredients: ["Fresh Atlantic Salmon", "Organic Brown Rice", "Miso Glaze", "Edamame"],
...
function Home() {
  ...
  const handlePostClick = () => {
    if (isLoggedIn) {
      console.log("Proceed to post meal");
      navigate('/post'); 
    } else {
      alert("Please login first to post your meal!");
      navigate('/login');
    }
  };
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex-1 max-w-3xl flex border border-gray-200 rounded-full shadow-sm bg-white overflow-visible h-12">
            <div className="relative flex-1 border-r border-gray-100">
              <button 
                ...
                className="w-full h-full pl-10 pr-4 text-left text-sm flex items-center justify-between focus:outline-none"
              >
                ...
                <div className="absolute top-full left-0 w-64 bg-white border border-gray-100 mt-2 shadow-xl rounded-2xl py-3 z-50">
                  {USF_LOCATIONS.map(cafe => (
                    <label key={cafe} className="flex items-center px-5 py-2 hover:bg-orange-50 cursor-pointer">
                      <input 
                        type="checkbox" 
                        className="accent-[#ff9000] mr-3" 
                        checked={selectedCafes.includes(cafe)} 
                        onChange={() => toggleItem(cafe, selectedCafes, setSelectedCafes)} 
                      />
                  ...
                  {PRICE_RANGES.map(range => (
                    <label key={range} className="flex items-center px-5 py-2 hover:bg-orange-50 cursor-pointer">
                      <input 
                        ...
                        onChange={() => toggleItem(range, selectedPrices, setSelectedPrices)} 
                      />
            ...
            <div className="relative flex-[1.2]">
              ...
              <input 
                className="w-full h-full pl-10 pr-4 focus:outline-none text-xs" 
                placeholder="Search dish..." 
              />
            </div>
            
            <button className="bg-[#ff9000] text-white px-6 rounded-full m-1 font-bold hover:bg-[#e68200] text-xs transition-all active:scale-95">
              GO
            </button>
          </div>
          ...
          <div className="ml-6 flex items-center gap-3">
            {!isLoggedIn ? (
              <>
                <Link to="/login" className="text-xs font-bold text-gray-400 hover:text-[#ff9000] transition-all">
                Login
                </Link>
                <Link to="/signup" className="text-xs font-bold bg-black text-white px-5 py-2.5 rounded-full hover:bg-gray-800 transition-all shadow-sm">
                  Sign Up
                </Link>
              </>
            ) : (
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-10">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-[#ff9000] text-white px-6 py-3 rounded-full font-bold hover:scale-105 transition-all shadow-lg text-sm"
          >
              ...
              <div className="flex flex-col sm:flex-row h-full">
                ...
                <div className="p-8 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className="text-xl font-black text-gray-900 leading-tight group-hover:text-[#ff9000] transition-colors tracking-tight italic">
                      {dish.dishName}
                    </h3>
